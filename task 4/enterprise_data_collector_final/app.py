...
async def main() -> None:
    """Run the complete data collection and reporting workflow."""
    ...
    reports = {
        "api_response_time_report": {
            "collected_endpoints": len(config.api_endpoints),
            "successful_endpoints": len(payloads),
        },
        "success_vs_failure_report": {
            "success_count": len(payloads),
            "failure_count": len(config.api_endpoints) - len(payloads),
        },
        "data_volume_report": {
            "processed_record_count": len(records),
            "aggregate": processor.aggregate(records),
        },
